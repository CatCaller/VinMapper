...
namespace intel_driver
{
	...
	bool CallKernelFunction(T* out_result, uint64_t kernel_function_address, const A ...arguments) {
		...
		if (ntdll == 0) {
			VinLog(L"[-] Failed to load ntdll.dll" << std::endl); //never should happens
			return false;
		}
		...
		if (!NtAddAtom)
		{
			VinLog(L"[-] Failed to get export ntdll.NtAddAtom" << std::endl);
			return false;
		}
		...
		*(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
		...
		if (!kernel_NtAddAtom) {
			VinLog(L"[-] Failed to get export ntoskrnl.NtAddAtom" << std::endl);
			return false;
		}
		...
		if (original_kernel_function[0] == kernel_injected_jmp[0] &&
			...
			VinLog(L"[-] FAILED!: The code was already hooked!! another instance of VinMapper running?!" << std::endl);
			return false;
		}
