...
bool FixSecurityCookie(void* local_image, ULONG64 kernel_image_base)
{
	...
	{
		VinLog(L"[+] Load config directory wasn't found, probably StackCookie not defined, fix cookie skipped" << std::endl);
		return true;
	}
	...
	if (!stack_cookie)
	{
		VinLog(L"[+] StackCookie not defined, fix cookie skipped" << std::endl);
	...
	if (*(uintptr_t*)(stack_cookie) != 0x2B992DDFA232) {
		VinLog(L"[-] StackCookie already fixed!? this probably wrong" << std::endl);
		return false;
	}

	VinLog(L"[+] Fixing stack cookie" << std::endl);
...
bool ResolveImports(portable_executable::vec_imports imports) {
	for (const auto& current_import : imports) {
		ULONG64 Module = VinUtils::GetKernelModuleAddress(current_import.module_name);
		if (!Module) {
...
ULONG64 VinMapper::MapDriver(BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {

	const PIMAGE_NT_HEADERS64 nt_headers = portable_executable::GetNtHeaders(data);

	if (!nt_headers) {
		VinLog(L"[-] Invalid format of PE image" << std::endl);
		return 0;
	}

	if (nt_headers->OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
		VinLog(L"[-] Image is not 64 bit" << std::endl);
		return 0;
	}
	...
	if (!kernel_image_base) {
		VinLog(L"[-] Failed to allocate remote image in kernel" << std::endl);
	...
	do {
		VinLog(L"[+] Image base has been allocated at 0x" << reinterpret_cast<void*>(kernel_image_base) << std::endl);
		...
		if (destroyHeader) {
			...
			VinLog(L"[+] Skipped 0x" << std::hex << TotalVirtualHeaderSize << L" bytes of PE Header" << std::endl);
		}
		...
		{
			VinLog(L"[-] Failed to fix cookie" << std::endl);
			return 0;
		}

		if (!ResolveImports(portable_executable::GetImports(local_image_base))) {
			VinLog(L"[-] Failed to resolve imports" << std::endl);
		...
		if (!intel_driver::WriteMemory(realBase, (PVOID)((uintptr_t)local_image_base + (destroyHeader ? TotalVirtualHeaderSize : 0)), image_size)) {
			VinLog(L"[-] Failed to write local image to remote image" << std::endl);
			kernel_image_base = realBase;
			break;
		}
		...
		if (mode == AllocationMode::AllocateIndependentPages)
		{
			...
			for (int i = 0; i < nt_headers->FileHeader.NumberOfSections; i++) {
				...
				if (secSize <= 0) {
					VinLog(L"[*] Skipping empty section: " << (char*)sec->Name << std::endl);
					continue;
				}
				...
				VinLog(L"[+] Setting protection for section: "
					<< (char*)sec->Name
				...
				if (!intel_driver::MmSetPageProtection(secAddr, secSize, prot)) {
					VinLog(L"[-] Failed to set protection for section: " << (char*)sec->Name << std::endl);
				}
			}
		}
		...
		VinLog(L"[<] Calling DriverEntry 0x" << reinterpret_cast<void*>(address_of_entry_point) << std::endl);

		if (callback) {
			if (!callback(&param1, &param2, realBase, image_size)) {
				VinLog(L"[-] Callback returns false, failed!" << std::endl);
				kernel_image_base = realBase;
				break;
			}
		}
		...
		if (!intel_driver::CallKernelFunction(&status, address_of_entry_point, (PassAllocationAddressAsFirstParam ? realBase : param1), param2)) {
			VinLog(L"[-] Failed to call driver entry" << std::endl);
			kernel_image_base = realBase;
			break;
		}
		...
		VinLog(L"[+] DriverEntry returned 0x" << std::hex << status << std::endl);

		// Free memory
		if (free) {
			VinLog(L"[+] Freeing memory" << std::endl);
			...
			if (free_status) {
				VinLog(L"[+] Memory has been released" << std::endl);
			}
			else {
				VinLog(L"[-] WARNING: Failed to free memory!" << std::endl);
			}
		}
