...
LONG WINAPI SimplestCrashHandler(EXCEPTION_POINTERS* ExceptionInfo)
{
	if (ExceptionInfo && ExceptionInfo->ExceptionRecord)
		VinLog(L"[!!] Crash at addr 0x" << ExceptionInfo->ExceptionRecord->ExceptionAddress << L" by 0x" << std::hex << ExceptionInfo->ExceptionRecord->ExceptionCode << std::endl);
	else
		VinLog(L"[!!] Crash" << std::endl);
...
bool callbackExample(ULONG64* param1, ULONG64* param2, ULONG64 allocationPtr, ULONG64 allocationSize) {
	...
	VinLog("[+] Callback example called" << std::endl);
...
void PauseIfParentIsExplorer() {
	...
	if (parentPid == explorerPid) {
		VinLog(L"[+] Pausing to allow for debugging" << std::endl);
		VinLog(L"[+] Press enter to close" << std::endl);
		std::cin.get();
	}
}
...
int wmain(const int argc, wchar_t** argv) {
	...
	if (!std::filesystem::exists(driver_path)) {
		VinLog(L"[-] File " << driver_path << L" doesn't exist" << std::endl);
		PauseIfParentIsExplorer();
		return -1;
	}
	...
	if (!VinUtils::ReadFileToMemory(driver_path, &raw_image)) {
		VinLog(L"[-] Failed to read image to memory" << std::endl);
		intel_driver::Unload();
		PauseIfParentIsExplorer();
		return -1;
	}

	VinMapper::AllocationMode mode = VinMapper::AllocationMode::AllocatePool;

	if (indPagesMode) {
		mode = VinMapper::AllocationMode::AllocateIndependentPages;
	}

	NTSTATUS exitCode = 0;
	if (!VinMapper::MapDriver(raw_image.data(), 0, 0, free, !copyHeader, mode, passAllocationPtr, callbackExample, &exitCode)) {
		VinLog(L"[-] Failed to map " << driver_path << std::endl);
		intel_driver::Unload();
		PauseIfParentIsExplorer();
		return -1;
	}
	...
	if (!NT_SUCCESS(intel_driver::Unload())) {
		VinLog(L"[-] Warning failed to fully unload vulnerable driver " << std::endl);
		PauseIfParentIsExplorer();
	}
	VinLog(L"[+] success" << std::endl);
