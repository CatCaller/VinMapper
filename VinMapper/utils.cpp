...
std::wstring VinUtils::GetFullTempPath() {
	...
	if (!get_temp_path_ret || get_temp_path_ret > MAX_PATH + 1) {
		VinLog(L"[-] Failed to get temp path" << std::endl);
		return L"";
	}
...
bool VinUtils::ReadFileToMemory(const std::wstring& file_path, std::vector<BYTE>* out_buffer) {
...
bool VinUtils::CreateFileFromMemory(const std::wstring& desired_file_path, const char* address, size_t size) {
...
uint64_t VinUtils::GetKernelModuleAddress(const std::string& module_name) {
	void* buffer = nullptr;
	DWORD buffer_size = 0;
...
BOOLEAN VinUtils::bDataCompare(const BYTE* pData, const BYTE* bMask, const char* szMask) {
...
uintptr_t VinUtils::FindPattern(uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, const char* szMask) {
...
PVOID VinUtils::FindSection(const char* sectionName, uintptr_t modulePtr, PULONG size) {
...
std::wstring VinUtils::GetCurrentAppFolder() {
	wchar_t buffer[1024];
